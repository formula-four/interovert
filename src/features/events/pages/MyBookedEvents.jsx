...
function BookingMeta({ joinedAt, paymentStatus }) {
  const dateLabel = joinedAt
  ...
  return (
    <div className="mb-3 flex flex-wrap items-center gap-2">
      {dateLabel ? (
        <span className="inline-flex items-center gap-1.5 rounded-full bg-zinc-800/90 px-2.5 py-1 text-xs font-medium text-zinc-300 ring-1 ring-zinc-600/50">
...
export default function MyBookedEvents() {
  ...
  const load = useCallback(async (pageNum) => {
    setLoading(true)
    try {
      const { data } = await apiClient.get('/api/events/me/bookings', {
    ...
    <div className="relative min-h-screen overflow-hidden bg-zinc-950 pt-28 text-zinc-100">
      ...
      <div className="relative mx-auto max-w-7xl px-4 pb-20 sm:px-6 lg:px-8">
        ...
        >
          ...
          <p className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.2em] text-emerald-400/90">
            <Ticket className="h-3.5 w-3.5" aria-hidden />
            Your spots
          </p>
          <h1 className="mt-2 text-4xl font-bold tracking-tight text-white sm:text-5xl">My booked events</h1>
          <p className="mt-3 max-w-2xl text-base leading-relaxed text-zinc-400">
            Free joins and completed ticket payments appear here. Pending payments are not listed until they are
            confirmed.
          </p>
        </motion.header>
        ...
        {loading ? (
          ...
          >
            <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-emerald-500/15 text-emerald-400 ring-1 ring-emerald-500/25">
              <Ticket className="h-7 w-7" />
            </span>
            <h2 className="mt-6 text-lg font-semibold text-white">
              {totalCount > 0 ? 'No bookings on this page' : 'No bookings yet'}
            </h2>
            <p className="mt-2 max-w-sm text-sm text-zinc-500">
              {totalCount > 0
                ? 'Try an earlier page.'
                : 'Join a free event or complete checkout on a paid event — it will show up here.'}
            </p>
            {totalCount > 0 ? (
              <button
                type="button"
          ...
          <>
            <ul className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
              {items.map((row, i) =>
                row.event ? (
                  <li key={`${String(row.event._id)}-${i}`}>
                    <BookingMeta joinedAt={row.joinedAt} paymentStatus={row.paymentStatus} />
                    <EventCard event={row.event} index={i} onGuestClick={promptAuth} />
                  </li>
                ) : null
              )}
            </ul>
